...
env.roledefs = {
    'myserver': ['wanpeng@127.0.0.1'],
}
env.PROJECT_NAME = 'blog'
env.SETTINGS_BASE = 'blog/blog/settings/base.py'
env.DEPLOY_PATH = '/home/wanpeng/.virtualenv/django-test'
# env.VENV_ACTIVATE = os.path.join(env.DEPLOY_PATH, 'bin', 'activate')
env.PYPI_HOST = '127.0.0.1'
env.PYPI_INDEX = 'http://127.0.0.1:9090/simple'
env.PROCESS_COUNT = 2
env.PORT_PREFIX = 909
...
def _ensure_virtualenv():
    ...
    if exists(env.DEPLOY_PATH):
        run('rmvirtualenv %s'% env.DEPLOY_PATH)
        # run('mkdir -p %s' % env.DEPLOY_PATH)
    run('mkvirtualenv %s' % env.DEPLOY_PATH)
...
def deploy(version, profile):
    ...
    with prefix('workon %s' % env.DEPLOY_PATH.split('/')[-1]):
